...
class TimerUsageControl:
    ...
    def handle_config_update(self, topic, payload):
        ...
        try:
            ...
            # Extract requester from topic: Group17/SmartChill/TimerUsageControl/{requester}/config_update
    ...
    def send_config_data(self, device_id, data_type, config, original_topic):
        ...
        data_topic = f"Group17/SmartChill/TimerUsageControl/{requester}/config_data"
        data_payload = {
            "device_id": device_id,
            "data_type": data_type,
            "config": config,
    ...
    def validate_config_values(self, config):
        ...
        if "max_door_open_seconds" in config:
            value = config["max_door_open_seconds"]
            if not isinstance(value, int) or value < 30 or value > 300:
                return "max_door_open_seconds must be an integer between 30 and 300"
        
        # Check check_interval
        if "check_interval" in config:
            value = config["check_interval"]
            if not isinstance(value, int) or value < 1 or value > 30:
                return "check_interval must be an integer between 1 and 30"
        
        # Check enable_door_closed_alerts
        if "enable_door_closed_alerts" in config:
            value = config["enable_door_closed_alerts"]
            if not isinstance(value, bool):
                return "enable_door_closed_alerts must be a boolean (true/false)"
        
        # Check for unknown config keys
        allowed_keys = {"max_door_open_seconds", "check_interval", "enable_door_closed_alerts"}
    ...
    def send_config_ack(self, device_id, status, config, original_topic):
        ...
        ack_topic = f"Group17/SmartChill/TimerUsageControl/{requester}/config_ack"
        ack_payload = {
            "device_id": device_id,
            "status": status,
            "config": config,
    ...
    def send_config_error(self, error_code, error_message, original_topic, device_id=None):
        ...
        error_topic = f"Group17/SmartChill/TimerUsageControl/{requester}/config_error"
        error_payload = {
            "device_id": device_id,
        ...
        try:
            ...
            print(f"[CONFIG] Sent ERROR: {error_code} - {error_message}, e: {error_topic}")
        except Exception as e:
