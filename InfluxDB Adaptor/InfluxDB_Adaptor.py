...
class InfluxDBAdaptor:
    ...
    def create_influx_point(self, device_id, sensor_type, value, timestamp):
    ...
    def store_sensor_data(self, device_id, sensor_type, value, timestamp):
        ...
        try:
            ...
            point = self.create_influx_point(device_id, sensor_type, value, timestamp)
            if not point:
                return False
    ...
    def query_door_events_from_influx(self, device_filter=None, duration="168h", *, limit=None):
        ...
        Args:
            device_filter (str|None): device_id to filter
            duration (str): time window (e.g., "24h", "7d")
            limit (int|None): max number of events to return
            
        Returns:
            {"events": [{"timestamp": unix, "event_type": str, "duration": float|None, "device_id": str}]}
        """
        try:
            bucket = self.settings["influxdb"]["bucket"]
            ...
            # We want both value and duration_seconds fields
            query += '''
            ...
            # Process results - group by timestamp and event_type
            events_data = {}  # {(timestamp, device_id, event_type): {"value": x, "duration": y}}
...
class InfluxRestAPI:
    ...
    def events(self, **params):
        ...
        try:
            ...
            duration = params.get("last", "168h")  # Default 1 week
    ...
    def sensors(self, sensor_type, **params):
        ...
        try:
            ...
            # Convert limit a int se presente
            if limit is not None:
                try:
                    limit = int(limit)
                except ValueError:
            ...
            # Validate duration format solo se non si usa last_only
